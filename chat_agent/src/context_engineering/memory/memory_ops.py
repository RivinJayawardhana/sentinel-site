...
class MemoryRecaller:
    ...
    def recall(
        ...
        # Short-term memory is best-effort and should not be blocked by LT failures.
        try:
            st_turns = self.st_store.recent(user_id, session_id, k_st)
        except Exception as e:
            logger.warning(f"Short-term recall failed: {e}")
            st_turns = []
        ...
        try:
            ...
            logger.warning(f"Long-term recall failed: {e}")
            lt_facts = []
