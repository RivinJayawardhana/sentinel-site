...
import { Download, Users, ShieldAlert, AlertTriangle, Activity, Brain } from "lucide-react";
...
const Analytics = () => {
  ...
  // Fetch history only for the selected worker
  const { data: workerHistory } = useEmployeeHistory(
    workerFilter !== "all" ? workerFilter : "",
    120,
  );

  // Merge ML/threshold alerts with backend alerts (deduplicated)
  const backendAlerts = data?.alerts ?? [];
  const combinedAlerts = useMemo(() => {
    return [...allAlerts, ...backendAlerts].reduce<typeof allAlerts>((acc, a) => {
  ...
  // Filtered alerts by time range + worker/zone/status filters
  ...
  const mlActiveAlerts = filteredAlerts.filter(a => a.status === "active" && (a.source === "ml" || a.source === "threshold")).length;
  const avgHR          = filteredWorkers.length
    ? Math.round(filteredWorkers.reduce((s, w) => s + w.heartRate, 0) / filteredWorkers.length)
    : 0;

  // ── Vital signs trend (selected worker) ─────────────────────────────────────
  ...
  // Current HR comparison bar (all workers mode)
  ...
  const severityData = useMemo(() => {
    ...
    return Object.entries(s)
      .filter(([, v]) => v > 0)
      .map(([sev, count]) => ({ name: sev.charAt(0).toUpperCase() + sev.slice(1), value: count, fill: SEVERITY_FILL[sev] }));
  }, [filteredAlerts]);

  // ── Risk distribution (linked to filters) ───────────────────────────────────
  ...
  // ── Scatter data with status colours ────────────────────────────────────────
  const scatterNormal   = filteredWorkers.filter(w => w.status === "normal").map(w =>   ({ name: w.name, temperature: w.temperature, heartRate: w.heartRate, id: w.id }));
  const scatterWarning  = filteredWorkers.filter(w => w.status === "warning").map(w =>  ({ name: w.name, temperature: w.temperature, heartRate: w.heartRate, id: w.id }));
  const scatterCritical = filteredWorkers.filter(w => w.status === "critical").map(w => ({ name: w.name, temperature: w.temperature, heartRate: w.heartRate, id: w.id }));
  ...
  const handleExport = () => {
    const rows = [
      ...
      ...leaderboard.map(r =>
        ...
        `${filteredWorkers.find(w => w.id === r.id)?.heartRate ?? ""},` +
        `${filteredWorkers.find(w => w.id === r.id)?.temperature ?? ""},` +
        `${filteredWorkers.find(w => w.id === r.id)?.airQuality ?? ""},` +
        `${r.active},${r.critical}`
      ),
    ].join("\n");
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardContent className="flex items-center gap-3 p-4 flex-wrap">
            ...
            <Button variant="ghost" size="sm" onClick={() => { setTimeRange("7d"); setWorkerFilter("all"); setZoneFilter("all"); setStatusFilter("all"); }}>
              Clear
            </Button>
        ...
        {/* Alerts Over Time + Type Breakdown */}
        <div className="grid grid-cols-2 gap-6">
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-sm">Alerts Over Time</CardTitle></CardHeader>
            <CardContent className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dailyAlertData}>
                  ...
                  <Bar dataKey="backend"   name="Backend"   stackId="a" fill="hsl(210, 40%, 60%)" />
                  <Bar dataKey="threshold" name="Threshold" stackId="a" fill="hsl(45, 96%, 56%)" />
                  <Bar dataKey="ml"        name="ML"        stackId="a" fill="hsl(28, 100%, 48%)" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-sm">Alert Type Breakdown</CardTitle></CardHeader>
            <CardContent className="h-56">
              {alertTypeData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={alertTypeData} layout="vertical" margin={{ left: 8 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                    <XAxis type="number" tick={{ fontSize: 10 }} stroke="hsl(var(--muted-foreground))" allowDecimals={false} />
                    <YAxis type="category" dataKey="type" tick={{ fontSize: 10 }} stroke="hsl(var(--muted-foreground))" width={88} />
                    <Tooltip contentStyle={{ backgroundColor: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: "8px", fontSize: "11px" }} />
                    <Bar dataKey="count" fill="hsl(28, 100%, 48%)" radius={[0, 4, 4, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
        ...
        {/* Severity Distribution + Scatter */}
        <div className="grid grid-cols-2 gap-6">
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-sm">Alert Severity Distribution</CardTitle></CardHeader>
            <CardContent className="h-56">
              {severityData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={severityData} cx="50%" cy="50%" innerRadius={45} outerRadius={85}
                      dataKey="value"
          ...
          <Card>
            ...
            <CardContent className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart>
                  <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                  <XAxis dataKey="temperature" name="Temp (°C)" tick={{ fontSize: 10 }} stroke="hsl(var(--muted-foreground))" label={{ value: "Temp °C", position: "insideBottom", offset: -2, fontSize: 10 }} />
                    ...
                    content={({ payload }) => {
                      ...
                      return (
                        <div className="rounded-lg border bg-card p-2 text-xs space-y-0.5">
                          ...
                          <p>HR: {d.heartRate} BPM</p>
                        </div>
                      );
                    }}
                  />
                  <Legend wrapperStyle={{ fontSize: "11px" }} />
                  <Scatter name="Normal"   data={scatterNormal}   fill={STATUS_FILL.normal}   onClick={(p: any) => p?.id && setWorkerFilter(p.id)} />
                  <Scatter name="Warning"  data={scatterWarning}  fill={STATUS_FILL.warning}  onClick={(p: any) => p?.id && setWorkerFilter(p.id)} />
                  <Scatter name="Critical" data={scatterCritical} fill={STATUS_FILL.critical} onClick={(p: any) => p?.id && setWorkerFilter(p.id)} />
                </ScatterChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
        ...
        <Card>
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground">Sorted by active alerts · {timeRange} window</span>
            </div>
          </CardHeader>
          <CardContent>
                  ...
                  {leaderboard.map(row => {
                    ...
                    return (
                      <tr
                        ...
                        onClick={() => setWorkerFilter(workerFilter === row.id ? "all" : row.id)}
                      >
                        <td className="py-2 font-medium">{row.name}</td>
                        <td className="py-2 text-muted-foreground text-xs">{row.zone}</td>
                        <td className="py-2">
                          <Badge className={`text-[10px] py-0 px-1.5 ${row.status === "critical" ? "bg-critical text-critical-foreground" : row.status === "warning" ? "bg-warning text-warning-foreground" : "bg-success text-success-foreground"}`}>
                        ...
                        <td className="py-2 text-right text-xs text-muted-foreground">
                          {row.lastAlert ? new Date(row.lastAlert).toLocaleString([], { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" }) : "—"}
                        </td>
                      </tr>
                    );
                  })}
